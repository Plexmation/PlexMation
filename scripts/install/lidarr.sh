...
sudo wget -t 0 -T 15 -P /opt/ProgramData $( curl -s https://api.github.com/repos/lidarr/Lidarr/releases | grep linux.tar.gz | grep browser_download_url | head -1 | cut -d \" -f 4 )
sudo tar -xvzf /opt/ProgramData/Lidarr.develop.*.linux.tar.gz -C /opt
sudo rm -rf /opt/ProgramData/Lidarr.develop.*.linux.tar.gz
